...
#### Feature Extraction
The *curse of dimensionality* might impose many difficulties on the task of anomaly detection. Traditionally used as a dimensinality reduction mechanism, autoencoders provide a automatic solution of this issue. practically speaking, instead of feeding anomaly detectors with the original signal, one can use the encoded form generated by AE as an input. Alongside the output layer, *FC_autoencoder()* returns also the hidden layer values as an output. Thus, composing the AE with the FCN might yield fruitful end-to-end solution.  
